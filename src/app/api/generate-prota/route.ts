...
import { generateProta } from "@/lib/mistral";
  ...
  try {
    ...
    if (!mataPelajaran || !fase || !kelas || kelas.length === 0) {
      return NextResponse.json(
        { error: "Mata pelajaran, fase, dan kelas wajib diisi" },
        { status: 400 }
      );
    }
    ...
    const protaOutput = await generateProta({
      mataPelajaran,
      fase,
      kelas,
      namaGuru,
    ...
    if (userId) {
      await prismaClient.prota.create({
        data: {
          userId,
          mataPelajaran,
          fase,
          ...
          rawOutput: protaOutput as any,
        },
      });
    }

    return NextResponse.json({ success: true, data: protaOutput });
  } catch (error) {
    console.error("Generate Prota error:", error);
    return NextResponse.json(
      { error: "Gagal generate Prota. Silakan coba lagi." },
      { status: 500 }
    );
  }
}